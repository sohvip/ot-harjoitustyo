...
class UI:
    ...
    def press_login(self):
        correct = not_empty(self.current.username.get(), self.current.password.get())
        if correct:
            correct_2 = self.account.find_account(self.current.username.get(), self.current.password.get())
            if correct_2:
                self.root.destroy()
                start = Start()
                start.start()
    ...
    def press_create(self):
        correct = not_empty(self.current.username.get(), self.current.password.get())
        if correct:
            correct_2 = self.account.new_account(self.current.username.get(), self.current.password.get())
            if correct_2:
                self.sign_in_view()
